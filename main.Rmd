...
## The Difference in Lab & Wild genes for a given Strain
```{r}
chromo <- "chr06"
mygeno <- genotype[, round(mean(which(marker$chrom==chromo)))]
names(mygeno) <- genotype$strain

attach(growth)
par(mfrow=c(1,5))
plot(growth$YPD ~ mygeno[strain], data=growth, main=chromo)
plot(growth$YPD_BPS ~ mygeno[strain], data=growth, main=chromo)
plot(growth$YPD_Rapa ~ mygeno[strain], data=growth, main=chromo)
plot(growth$YPE ~ mygeno[strain], data=growth, main=chromo)
plot(growth$YPMalt ~ mygeno[strain], data=growth, main=chromo)
```
