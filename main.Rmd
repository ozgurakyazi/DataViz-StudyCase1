...
## The Difference in Lab & Wild gene counts for a given chromosome
```{r}
given_chrom <- "seg_01B" ## The given chromosome...
sel_markers <- data.frame(Marker = marker_dt[chrom== given_chrom,]$id)
mygeno <- as.data.table(merge(genotype_melted, sel_markers,by= "Marker" )) ## merge, left join, on markers
ggplot(mygeno, aes(Source)) + geom_histogram(aes(as.factor(mygeno$Source))) + facet_wrap(mygeno$Marker)
